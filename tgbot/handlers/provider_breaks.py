...
from utils.bot.consts import DATE_FORMAT, DEFAULT_SLOT, TIME_FORMAT, TIME_INPUT_FORMAT, WDS, WDS_REV, WEEKDAYS
...
async def edit_lunch_start(message: Message, state: FSMContext):
    ...
    markup.keyboard.append([_("Clear recurring lunch time")])
    markup.keyboard.append([_("Back to breaks & days off")])
    await state.set_state(BreaksStatesGroup.edit_lunch_start)
    reply_message = _("Enter the desirable lunch start time in HH:MM format")
    await message.answer(reply_message, reply_markup=markup)


@provider_breaks_router.message(F.text.in_([_("Clear recurring lunch time")]), IsProviderFilter())
async def clear_lunch(message: Message, state: FSMContext):
    await update_provider(message.from_user.id, lunch_start=None, lunch_end=None)
    await state.clear()
    await message.answer(
        _("You have cleared your daily lunch hours."), reply_markup=get_provider_breaks_days_off_menu()
    )
...
async def edit_lunch_end(message: Message, state: FSMContext):
    ...
    if (not 0 <= int(h) <= 23) or (m not in ["00", "30"]):
        markup = ReplyKeyboardMarkup(keyboard=[[]], resize_keyboard=True)
        markup.keyboard.append([_("Back to breaks & days off")])
        await message.answer(
            _("Please, enter time in HH:MM format with MM being among 00 and 30"),
            reply_markup=markup,
        )
        return
...
async def finish_edit_lunch(message: Message, state: FSMContext):
    ...
    if (not 0 <= int(h) <= 23) or (m not in ["00", "30"]) or (start >= end):
        markup = ReplyKeyboardMarkup(keyboard=[[]], resize_keyboard=True)
        markup.keyboard.append([_("Back to breaks & days off")])
        await message.answer(
            _("Please, enter end time (bigger than the start time) in HH:MM format with MM among 00 and 30."),
            reply_markup=markup,
        )
        return
...
@provider_breaks_router.message(
    or_f(F.text.in_([_("Edit weekly days off")]), BreaksStatesGroup.edit_days_off), IsProviderFilter()
)
...
async def set_a_break_duration(message: Message, state: FSMContext):
    ...
    await message.answer(
        _("Choose an option or enter the duration of a break in minutes (should be divisible by 30)."),
        reply_markup=markup,
    )
...
async def set_a_break_time(message: Message, state: FSMContext):
    ...
    if not state_data.get("duration"):
        try:
            ...
            if duration % DEFAULT_SLOT != 0:
                while (duration % DEFAULT_SLOT) != 0:
                    duration += 1
